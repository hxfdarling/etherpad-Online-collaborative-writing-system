...
Pad.prototype.appendRevision = function appendRevision(aChangeset, author) {
    ...
    newRevData.meta.cmd = "add";
    newRevData.meta.sumCount = Changeset.newLen(aChangeset);//当前文档中的字数
    if (aChangeset) {
        if (remove.test(aChangeset) && !replace.test(aChangeset)) {
            newRevData.meta.cmd = "remove";
            this.removeCount++;
        } else if (replace.test(aChangeset)) {
            newRevData.meta.cmd = "replace";
            this.replaceCount++;
        } else {
            newRevData.meta.cmd = "add";
            this.addCount++;
...
//新添加的查询方法
Pad.prototype.getAllAuthorsObj = function (callback) {
    var authors = [], key, returnAuthors = [];
    authors = this.getAllAuthors();
    async.forEach(authors, function (author, callback) {
        authorManager.getAuthor(author, function (err, authorsObj) {
            if (err) {
                return callback(err);
            }
            returnAuthors.push({id: author, name: authorsObj.name});
            callback();
        });
    }, function (err) {
        callback(err, returnAuthors);
    });
}
...
Pad.prototype.addSavedRevision = function addSavedRevision(revNum, savedById, label) {
    console.info("addSaveRevision" + revNum);
    ...
    savedRevision.addCount = this.addCount;
    savedRevision.removeCount = this.removeCount;
    savedRevision.replaceCount = this.replaceCount;
...
Pad.prototype.getLastVersionsByUser = function getLastVersionsByUser(author, callback) {
    console.info("你访问了getLastVersionsByUser Function");
    var versions = [];
    var curRev = 0;
    var _this = this , head = this.getHeadRevisionNumber();
    while (curRev <= head) {
        versions.push(head);
        head--;
    }
    ...
    async.series(
        [
            function (callback) {
                async.forEach(versions, function (item, callback) {
                    db.getSub("pad:" + _this.id + ":revs:" + item, ["meta"], function (err, data) {
                        if (ERR(err, callback)) return;
                        data = data ? data : {};
                        data.revs = item;
                        for (var i = 0; author && i < author.length; i++) {
                            if (data.author && data.author == author[i].id && !author[i].flag) {
                                data.author = author[i];
                                targetVersion.push(data);
                                author[i].flag = true;
                            }
                        }
                        callback();
...
Pad.prototype.getAllVersions = function getAllVersions(callback) {
    var versions = [];
    var curRev = 0;
    var _this = this, head = this.getHeadRevisionNumber();
    while (curRev <= head) {
        versions.push(curRev);
        curRev++;
    }
    var versionInfo = [];
    versionInfo.padID = _this.id;
    async.series(
        [
            function (callback) {
                async.forEach(versions, function (item, callback) {
                    db.get("pad:" + _this.id + ":revs:" + item, function (err, data) {
                        if (ERR(err, callback)) return;
                        data = data ? data : {};
                        data.meta.revs = item;
                        var t = data.changeset || "", a;
                        a = t.substring(t.indexOf("$"));
//                        b = t.substring(0, t.indexOf("$"));
//                        t = b = b.match(/-[\d]{1,}/);
//                        b = b ? parseInt(b) : 0;
//                        console.error(data.meta.sumCount);

                        data.meta.textCount = a.length;
                        versionInfo.push(data.meta);
                        callback();
                    });
                });
                callback();
...
Pad.prototype.getVersionsByTime = function getVersionsByTime(start, end, callback) {
    var versions = [];
    var curRev = 0;
    var _this = this, head = this.getHeadRevisionNumber();
            ...
            function (callback) {
                async.forEach(versions, function (item, callback) {
                    db.getSub("pad:" + _this.id + ":revs:" + item, ["meta"], function (err, data) {
                        ...
                        data = data ? data : {};
                        data.revs = item;
                        if (data.timestamp && data.timestamp >= start & data.timestamp < end) {
