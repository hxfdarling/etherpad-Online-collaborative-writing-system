 ...
 * @param callback
 */
exports.listAuthorsObjOfPad = function(padID,callback){
    //get the pad
    console.info("访问了listAuthorsObjOfPad方法");
    getPadSafe(padID, true, function(err, pad)
    {
        if(ERR(err, callback)) return;
        pad.getAllAuthorsObj(callback);
    });
}
/**
...
/*****************/
/**core function项目核心算法调用*/
/*****************/

/**
 * 获取目标pad所有版本号，以及版本号创建者
 * @param padID
 * @param callback
 */
exports.getAllVersions = function (padID, callback) {
    //get the pad
    getPadSafe(padID, true, function (err, pad) {
        pad.getAllVersions(function (versionInfo) {
            callback(null, {version: versionInfo});
        });
    });
}
exports.getVersionsByTime = function (padID, start, end,callback) {
    getPadSafe(padID, true, function (err, pad) {
        pad.getVersionsByTime(start, end, function (versionInfo) {
            callback(null, {version: versionInfo});
        });
    });
}
exports.getLastVersionsByUser = function(padID,author,callback){
    getPadSafe(padID, true, function (err, pad) {
        pad.getLastVersionsByUser(author, function (versionInfo) {
            callback(null, {version: versionInfo});
        });
    });
}
